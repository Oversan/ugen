...
function createConfig(boilerplatePath, callback) {
  const options = {
    cwd: boilerplatePath,
    dot: true,
    ignore: '**/node_modules/**/*'
  ...
  glob('**/*.template', options, function(err, files) {
    ...
    writeFile(path.join(process.cwd(), 'ugen.config.js'), configTemplate({questions: questions}, {templatesVariables: templatesConfigVariables}))
